# Blind Hunter Review Prompt (Story 3-3)

Use the `bmad-review-adversarial-general` skill.

Constraints:
- You receive ONLY the diff below.
- No spec, no repository lookups, no additional context.
- Find concrete bugs, regressions, risky assumptions, and semantic breakage visible from this diff alone.
- Output findings as markdown bullets with: severity (`high|medium|low`), title, evidence (file/hunk), and impact.
