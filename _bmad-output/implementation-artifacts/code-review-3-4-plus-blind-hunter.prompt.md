...
## Constraints
- Use only the diff below.
- Do not use repository context, spec files, or assumptions outside the diff.
- Output findings only as a Markdown list.
- For each finding include: severity (`high|medium|low`), one-line title, evidence (file + hunk summary), and concrete risk.
