# Edge Case Hunter Review Prompt (Story 3-3)

Use the `bmad-review-edge-case-hunter` skill.

Constraints:
- You receive the diff below.
- You MAY inspect project files read-only for context.
- Focus on branching paths, boundary conditions, replay/idempotency, state-machine transitions, and failure handling.
- Output findings as markdown bullets with: severity (`high|medium|low`), title, edge case, evidence (file/hunk), and impact.
